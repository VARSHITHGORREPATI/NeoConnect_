...
import { FolderOpenDot, AlertTriangle, Hash, MapPin, Tag, Briefcase } from "lucide-react";
    ...
    <DashboardShell>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-900">Assigned Workflow</h1>
            <p className="text-sm text-slate-500 mt-1">Manage and resolve cases assigned explicitly to your queue.</p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          {loading ? (
             <div className="p-12 text-center flex flex-col items-center justify-center space-y-3">
               ...
               <p className="text-sm font-medium text-slate-500 animate-pulse">Fetching assigned cases...</p>
             </div>
          ) : cases.length === 0 ? (
            <div className="p-16 text-center flex flex-col items-center justify-center space-y-4">
              ...
              <div>
                <p className="font-bold text-lg text-slate-900">Inbox Zero</p>
                <p className="text-sm font-medium text-slate-500 mt-1">You have no active cases assigned to your queue.</p>
              </div>
            </div>
          ) : (
              ...
              <table className="w-full text-left text-sm whitespace-nowrap">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Current Status</th>
                  </tr>
                </thead>
                    ...
                    <tr key={c._id} className="hover:bg-slate-50/50 transition-colors group">
                      <td className="px-6 py-4">
                         <div className="flex flex-col">
                           ...
                           <span className="text-xs font-medium text-slate-500 mt-1 uppercase tracking-wider">Ticket Reference</span>
                         </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                         <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-bold uppercase tracking-wider shadow-sm ring-1 ring-inset ${getSeverityBadge(c.severity)}`}>
                           {c.severity === 'high' && <AlertTriangle className="w-3 h-3" />}
                           {c.severity}
                         </span>
                      </td>
                      <td className="px-6 py-4 text-right">
                         ...
                         <div className="mt-2 text-xs font-medium text-blue-600 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer flex items-center justify-end gap-1">
                           Manage Record <Briefcase className="w-3 h-3" />
                         </div>
                      </td>
                    </tr>
                  ))}
