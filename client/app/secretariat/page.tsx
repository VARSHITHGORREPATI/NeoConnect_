...
import { FolderOpenDot, Hash, MapPin, Tag, Briefcase, AlertTriangle } from "lucide-react";
    ...
    <DashboardShell>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-900">Secretariat Inbox</h1>
            <p className="text-sm text-slate-500 mt-1">Global view of all submitted cases across the organization.</p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          {loading ? (
             <div className="p-12 text-center flex flex-col items-center justify-center space-y-3">
               ...
               <p className="text-sm font-medium text-slate-500 animate-pulse">Fetching global cases...</p>
             </div>
          ) : cases.length === 0 ? (
            <div className="p-16 text-center flex flex-col items-center justify-center space-y-4">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-slate-500 mt-1">There are no active cases in the system yet.</p>
              </div>
            </div>
          ) : (
                    ...
                    <tr key={c._id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1.5">
                          ...
                          <span className="inline-flex items-center gap-1.5 text-xs font-medium text-slate-500">
                            <MapPin className="w-3.5 h-3.5 text-slate-400" /> {c.department}
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-right">
                         ...
                         <div className="mt-2 text-xs font-medium text-blue-600 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer flex items-center justify-end gap-1">
                           Review Details <Briefcase className="w-3 h-3" />
                         </div>
                      </td>
                    </tr>
                  ))}
